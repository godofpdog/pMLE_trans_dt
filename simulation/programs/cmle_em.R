   ...
   Lc=function(x) sum((exp(-x*nU)-x*nU*exp(-x*nU))/(x*exp(-x*nU))-(nT*exp(-x*nT)-ifelse((nT-d0)>=0,nT-d0,0)*exp(-x*ifelse((nT-d0)>=0,nT-d0,0)))/(exp(-x*ifelse((nT-d0)>=0,nT-d0,0))-exp(-x*nT)))
   theta=nleqslv(x=sp,Lc)$x
